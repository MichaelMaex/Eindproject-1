...
This project revolves around one of Kaplansky's conjectures. 
He conjectured that all group rings are directly finite, i.e. that left invertible elements are right invertible as well. 
This conjecture is still open, but the case has been solved for a very large class of groups, the sofic groups, which are defined in section \cref{sec:sofic_group}. 
In fact this class is so large that we do not know whether any non-sofic group exists (citatie naar Wiess toevoegen)\cite{}. 
...
\begin{figure}[ht]
    \centering
    \incfig{folner}
    \caption{An illustration of the Folner criterion. The limit means that the gray part will be come very small compared to  $F_n$ itself}
