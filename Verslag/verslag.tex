...
\subsection{Definition and examples}
	...
	\end{definition}
	We are now able to state the definition of sofic groups.
	\begin{definition}
		...
		\begin{itemize}
			\item $d_{S_n}\left( \Phi(gh), \Phi(g)\Phi(h) \right) < \epsilon$
	...
	\begin{definition}
		...
		\begin{itemize}
			\item $d_H(\Phi(gh),\Phi(g)\Phi(h)) < \epsilon$
...
\subsection{Properties of Residually Finite Groups}
    ...
    \begin{definition} % van wikipedia gehaald
	    Let $G$ be a group and $X$ be a set. Then $\alpha: G \times X \to X: (g,x) \mapsto \alpha_g(x)$ is called an textbf{action of $G$ on $X$} if
        \begin{enumerate}[(i)]
