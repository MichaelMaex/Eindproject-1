...
\begin{theorem}
    Let $\mathbb K$ be a field and $G$ be a finite group. Then $\mathbb K[G]$ is directly finite. % hier \mathbb F gebruiken omdat het veld eindig is?
\end{theorem}
\begin{proof}
...
In this section we will introduce the concept of a \emph{sofic group}. A sofic group is a group that is ``well approximated'' by a finite permutation group. Recall that every finite group is isomorphic with a permutation group.
...
\begin{definition} % Szabo: reference
...
$(\Phi_i \to \sym(d_i))_{i \in \N}$ %Szabo:  what does this notation mean?
which mimic the group structure of $G$ better and better in $\sym(d_i)$ as $i$ tends to infinity. %akward phrasing?
...
We can now state the definiton of a sofic group by the example of \cite{capraro_lupini_2015}.
    ...
    \begin{definition}\label{def:Sofic} % Szabo: more precise reference
    ...
    We will call such a family of maps $\left(\Phi_i : G \to \sym(d_i) \right)_{i \in \N}$ % Szabo: ?
...
If a countable group statisfies the Følner condition, we will also call the group \emph{amenable}.\footnote{One can also define amenability also for uncountable groups. In that case however, it is defined differently and the Følner criterion is not equivalent to the amenability.} % is dit wel juist? kerr-li ergodic theory lijkt te suggereren dat dit wel klopt!
...
\begin{example}\label{ex:integers_folner}
    ...
    \[
        ...
        \leq \frac{2k}{2i+1},
    \]
    and this converges to zero as $i \to \infty$. % Szabo: remember that k can be negative
\end{example}
...
% Szabo: Is this your theorem? If you present something like this which is well-known, either provide a reference or at least point out that it is known.

 	\begin{theorem}\label{thm:folner_sofic}
        Every countable group $G$ that satisfies the Følner criterion is sofic.
    \end{theorem}
    \begin{proof}
        \input{proof_folner}
 	\end{proof}
...
\subsubsection{About Følner Sequences} % deze sectie moet misschien gemerged worden met de vorige

In this section we list some more properties obout the Følner criterion and Følner sequences. These will be used in subsection \ref{ssec:}
...
\begin{example}
	...
	Consider $F_k = \left<x_1, x_2, x_3, \ldots, x_k \right>$. % Szabo: reference?
Let \[
...
We will construct a map from $F_k$ to $S_{N_r + 1}$, which is the symmetric group on $\{0,1, 2, \ldots, N_{r}\} $. % waarom hier andere notatie?
...
It is a well-known result that residually finite groups are sofic. % reference?
    ...
    % Deze sectie moet herwerkt worden naar een overview van groepen die sofisch zijn, met referenties naar de voorbeelden/stellingen waar ze werden aangehaald
    ...
    \end{definition} % Szabo: be precise
    ...
    Since $H$ is of finite index, this set is finite. % Szabo: Justify! This is by no means true for an arbitrary action on a space.
    ...
    % Szabo: Be more precise. In light of your previous remark you obviously don't mean what you write. You claim denseness for the set of all points which are periodic w.r.t. some H, or, the union of the H-periodic points over ALL H.
    \end{lemma}
    \begin{proof}
        ...
        % Szabo: "This means" needs more justification
        ...
        For every $F_n =  \{1, g_1, g_2, \dots, g_n\}$ we can construct such a function $\pi_n$ which is an $H$ periodic point % Szabo: H=?
...
%The phrasing here suggests that what you are about to do has something novel to it. Is that really the case? If so, point out explicitly what it is.
