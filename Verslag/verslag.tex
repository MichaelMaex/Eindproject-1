...
We now state the proof.
	...
	Take any  $g_i \in G$. To verify the first property of the Følner criterion, we can choose an $N$ sufficiently high such that $N >i$ and $g_i \in G_N$. Then $g_i \in \mathcal{F}_{m}$ for all $m > N$.
...
We will first need a couple of extra properties about the Følner criterion and Følner sequences.
...
\end{lemma}
\begin{proof}
	We'll prove it for odinary (left) Følner sequences as the proof is completely analogous for right Følner sequences.
...
\begin{lemma}\label{lem:folner_finite_subset}
	Let $F_n$ be a Følner sequence. Then for every finite subgroup $B \subset G$ it holds that  \[
...
\end{proof}
\begin{remark}
	A similar result holds for a right Følner sequence.
\end{remark}
