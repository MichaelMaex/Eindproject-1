...
To define this, we will need a way to measure a distance between permutations. It is quite natural to define the distance between as the fraction of elements on which they differ.
...
$(\Phi_i \to \sym(d_i))_{i \in \N}$ which mimic the group structure of $G$ better and better in $\sym(d_i)$ as $i$ tends to infinity. For that, we first of all want that $d_{\sym(d_i)}(\Phi_i(g) \circ \Phi_i(h), \Phi_i(gh))$ converges to zero, for every $g$ and $h$ in $G$. Additionaly, we want that in the limit these maps have a sort of injectivity property. Therefore we will require all elements of $G$ exept the identity element to converge out of the kernel of these maps. We add the following condition
\[
\lim_{i \to \infty} d_{\sym(d_i)}(\id,\Phi(g)) = \begin{cases}  0 & \text{if } g = e \\
                                        1 & \text{otherwise,}
                                        \end{cases}
\]
where $e$ denotes the identity element of $G$ and $\id$ is the identity permutation.\footnote{One might ask why we do not require that every $g \neq 0$ converges to some fixed strictly positive number $r(g)$ instead. In fact one can show that if that is the case, one can construct another sequence of maps $\tilde \Phi_i : G \to \sym{\tilde d_i}$ such that $d_{\sym \tilde d_i}(\id, \tilde \Phi_i(g))$ converges to $1$ for every $g$.}

We can now state the definiton of a sofic group.
    ...
    \begin{definition}\label{def:Sofic}
        ...
        \begin{enumerate}[(i)]
            \item $\lim_{i \to \infty} d_{\sym(d_i)}(\Phi_i(g) \circ \Phi_i(h), \Phi_i(gh)) = 0$
            \item $\lim_{i \to \infty} d_{\sym(d_i)}(\id,\Phi(g_i)) = \begin{cases}  0 & \text{if } g = e \\
                                                                                    1 & \text{otherwise,}
                                                                                    \end{cases}$
        \end{enumerate}
    ...
    %\input{sofic_groups_old_section}
...
We now state the proof.
    ...
    \begin{corollary}\label{cor:quotient_amenable}
	    Every quotient group of a amenable group is amenable. Every normal subgroup of an amenable group is amenable. 
    \end{corollary}
    \begin{proof}
	...
	For every $G_i$ we choose a Folner sequence $(F_{i, n})_n$. We will use these Folner sequences to construct a Folner sequence for $G$, $\mathcal{F}_n$.
	Let's fix an $n$, and consider the set $H = G_n \cap \{g_1, \ldots, g_n\}$. 
	As $H$ is a finite subset of $G_n$ we can find a Folner set $F_{n, N}$ such that for all $h \in H:$ 
	\begin{enumerate}
		\item $h \in F_{n, N}$
	...
	Now I claim that $\mathcal{F}_n$ is a Folner sequence of $G$. 
	Take any  $g_i \in G$. To verify the first property of the folner criterion, we can choose an $N$ sufficiently high such that $N >i$ and $g_i \in G_N$. Then $g_i \in \mathcal{F}_{m}$ for all $m > N$.

	To verify the second property we can again take $N$ such that $N > i$ and $g_i \in G_N$. For all $m >N$ we see that \[
	...
	.\] 
	We conclude that $\mathcal{F}_n$ is a Folner sequence and that  $G$ is amenable. 
    \end{proof}
...
\end{remark}
\subsection{Amenable groups are surjunctive}
...
This gives the following intermediate result, which will, in this paper, remain without proof. We refer again to \cite{capraro_lupini_2015}.
