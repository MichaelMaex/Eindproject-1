...
By the following lemma, we can not only exploit this\todo[color=green]{make precise} for all the elements $g \in G$ individually, but also for finite subsets $B \subset G$.
...
We will construct a map from $F_k$ to $\sym(N_r + 1)$, which is the symmetric group on $\{0,1, 2, \ldots, N_{r}\} $.  \todo{Szabo heeft hier problemen met de notatie, ik heb geen flauw idee waarom} % waarom hier andere notatie?
...
\[
	x_m \mapsto \ext_{\substack{j \in \{1,\ldots, r\} \\ m = i_j} } \begin{pmatrix} N_{j-1} & N_{j-1} +  1& \ldots & N_{j} -1 & N_{j} \end{pmatrix}^{\epsilon_j}% In this instance, this notation is really confusing. Better write it out.
,\] 
