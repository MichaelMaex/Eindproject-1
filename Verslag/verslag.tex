...
\todo{Szabo: If you go down the entropy route you need more precise references}
\begin{definition}
...
Suppose now that $G$ is sofic and $\Sigma := (\Phi_i: G \to \sym(d_i))_{i \in \N}$ is a sofic approximation sequence. Take a subshift $X$ and a finite subset $F \subset G$. For any $\delta > 0$ and $n \in \N$ we define the function \todo{Where does this definition come from?}
		...
		\]\todo{Distance between elements of different sets?}
		\todo{What is $g^{-1}(x_i)$?}
...
This gives the following intermediate result, which will, in this report, remain without proof. We refer again to \cite{capraro_lupini_2015}. \todo{Better reference}
