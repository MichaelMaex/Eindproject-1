...
We now state the proof.
    ...
    \end{proof}
 %\begin{theorem}
	 %The quotient of a countable amenable group is amenable.
 %\end{theorem}
 %\begin{proof}
	 %Let $G$ be a countable amenable group and let $H = \frac{G}{N}$ be a quotient group of $G$. 
	 %By the folner criterion $G$ has a folner sequence $F_n$. 
	 %I claim that the sequence $\overline{F}_n$ is a folner sequence of $H$. 
	 %The first property of the folner sequence obviously hold. 
	 %
	 %Take an $\overline{g} \in G$ and let's have a look at the limit
	 %\[
		 %\lim_{n \to \infty} \frac{\card{\overline{g}\overline{F}_n \triangle \overline{F}_n}}{\card{\overline{F}_n}} 
		 %\le \lim_{n \to \infty} \frac{\card{\overline{gF_n \triangle F_n}}}{\card{\overline{F}_n}}
	 %.\]
 %
 %\end{proof}
