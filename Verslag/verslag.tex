...
\todo{Szabo: Is this your theorem? If you present something like this which is well-known, either provide a reference or at least point out that it is known.}
    ...
    \subsection{About Følner Sequences} \todo[color=green]{merge this section with the previous one} 
...
\begin{proof}
 ...
 .\]
	...
	Define $\beta: \pi(F_n) \to A: h \mapsto \left(f \circ \left( \pi|_F \right)^{-1}\right)(h) $.
	Notice that for all $g_i \in F_n: (\beta \circ \pi)(g_i) = f(g_i)$.
	Take any extension  $\alpha: \im(\pi) \to A$ of  $\beta$.
...
\\
Suppose now that $G$ is sofic and $\Sigma := (\Phi_i: G \to \sym(d_i))_{i \in \N}$ is a sofic approximation sequence. Take a subshift $X$ and a finite subset $F \subset G$. For any $\delta > 0$ and $n \in \N$ we define the set \todo{Where does this definition come from?}
		
		\[
		H_n (\delta) := \{(a_i)_{0 \leq i \leq n-1} \in A^n | 
						\exists (x_i)_{0 \leq i \leq n-1} \in X_n : 
						\forall g \in F: d_{\sym(n)}((a_{\Phi^{-1}_n(g)(i)})_i,(x_{g g_i})_i) < \delta \}
		\]
		% \[
		...
		% \]
		where by $X_n \subset A^n$ we mean the sequences in $X$ restricted to $\{e, g_1, g_2, ..., g_{n-1}\}$.\\
...
We shall use the following intermediate result, for which a sketch of proof is provided. For the complete proof, we refer again to either \cite[paragraph 2.13.7]{capraro_lupini_2015} or \cite[theorem 4.12]{kerr_li_2010}.
...
\section{$\mathbb K^G$ is a $\mathbb K[G]$-module}
