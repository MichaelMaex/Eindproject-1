...
\todo{Why do we single out these fields?}
\end{figure}
...
Now for the second condition. It is clear that for any $i \in \N, k \in \{1,\dots, i\}$ : $\Phi_{i}(0)(k) = k$. Now take any $g \neq 0$ and consider $\Phi_{i}(g)$. For any $k \in \{1,\dots,i\}$ it is perfectly possible that $\Phi_{i}(g)(k) = k$. However this is not the case as soon as $i > |g|$. In that case, we have that $|(k+g)-k| < i$, from which it follows that $(k+g)\mod i \neq k \mod i$. Thus we have that $(k+g)\mod i \neq k$ (as $k = k\mod i$ for $k<i$) and the inequality is obvious for $k \geq i$. \todo{make this more clear} % misschien nog niet duidelijk genoeg
    ...
    \begin{definition}\cite{noauthor_folner_2019} \label{def:folner} \todo{Check is the "first condition" isn't used anymore}
...
% Szabo: Is this your theorem? If you present something like this which is well-known, either provide a reference or at least point out that it is known.
 	...
 	\begin{theorem}\label{thm:folner_sofic}
        Every amenable countable group $G$ is sofic.
    \end{theorem}
    \begin{proof}
        \input{proof_folner}
 	\end{proof}
