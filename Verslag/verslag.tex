...
\begin{lemma}\label{lem:right_folner_sequence}
	If a group $G$ satisfies the folner criterion, then there also exists a folner sequence for the right action of $G$ on itsself. 
...
\begin{lemma}\label{lem:folner_finite_subset}
	...
	\lim_{n \to \infty} \frac{\card{BF_n \triangle F_n}}{\card{F_n}}	
	.\] 
\end{lemma}
\begin{proof}
	TODO
...
In the following section $G$ will be a fixed amenable group, $A$ a fixed finite set and $\phi: A^{G} \to A^{G}$ a fixed countinuous injective function that commutes with the right shift. 
...
\end{lemma}
\begin{proof}
	Choose any $a \in A$. As $\phi$ is contunuous we know that \[
	\phi^{-1}\left( \prod_{g\in G} \begin{cases}
			\{a\} & g = 1\\
			A & g \ne 1
	\end{cases} \right)
\]
	is open. Hence is contains a subset  \[
	...
	We can create such  $\Gamma_a$ for every $a \in A$. If we take  $\Gamma = \bigcup_{a \in A} \Gamma_a$, we see that the value of $\phi((x_g)_g)(1)$ is only dependent on the values of  $x_h$ where $h \in Gamma$. 
\end{proof}
...
\begin{corollary}\label{cor:differ_in_image}
	If $(x_g)_g, (y_g)_g \in A^{G}$ only differ on a set $H \subset G$, ie $\forall g \in H:x_g = y_g$,
	then  $\phi((x_g))$ and $\phi((y_g))$ will only differ on $H\Gamma^{-1}$.
\end{corollary}
\begin{proof}
	Take $g \not\in H\Gamma^{-1}$. Then $\phi((x_h)_h)(g)$ will only depend on  $x_{g\gamma}$, where $\gamma \in\Gamma$. But  $g\Gamma \cap H = \emptyset$. By the previous corollary we see that $\phi((x_g)) = \phi((y_g))$.
\end{proof}

\begin{definition}
	We say that $(x_g)_g$ has support $H$ if $x_g = 0$ for all $g \not\in H$.
\end{definition}
We can implicitly identify all elements of $A^{G}$ with support $H$, with $A^{H}$.

\begin{lemma}\label{lem:injective_restriction}
	Let $H$ be any finite subgroup of $G$. Consider the function  \[
		\psi:A^H \to A ^{H\Gamma^{-1}}: (x_g) \mapsto (h \mapsto \phi((x_g)_g)(h)
	.\]  
	This function is a injection. 
\end{lemma}
\begin{proof}
	Suppose that $x= (x_g)_g, y = (y_g)_g \in A^{H}$ have the same image under $\psi$. As $x, y$ can only differ on $ H$ their images under $\phi$ cannot differ in $G\setminus H\Gamma^{-1}$. Because they also have the same image with $\psi$, we know that $\phi(x)$ and $\phi(y)$ must also agree on $H\Gamma^{-1}$. Hence $\phi(x) = \phi(y)$. By the injectivity of $\phi$ we find that $x = y$.
\end{proof}

At this point we're almost ready to prove that $G$ is surjunctive, i.e.\ $\phi$ is surjective. 
In oder to show this will show that for any finite subset $B\subset G$ and a map $f:B \to A$ we can find a $x \in A^{G}$ such that $\phi\left(x \right) |_B  = f$. Then we can finish the prove by showing that they image of $\phi$ is dense and using compactness.
...
We can scrap these elements form $H <- H \setminus \{g \in F | B\cdot g \cap B\cdot f \ne \emptyset\}$. Now we take a new  $f \in H$ and repeat the process.
We can repeat this atlest $\frac{\card{F}}{\card{B^2}}$ therefor $F'$ contains atleast as many elements. 
...
\begin{proof}
	...
	Notice that this meant that for no copy of $B$, take  $B\cdot S$, the image of $\phi$ does not contain a function  $\phi(x)$ such that when restrited to  $B\cdot s$ it is $f': B\cdot s \to A: x \mapsto f\left( x s^{-1} \right) $.
	...
	We know that for every such copy of $B$ the possible value at the coordinates of  is  $A^{\card{B}} -1$ by our assumption at the start of this proof.
...
This means that there exists a $x \in A^{G}$ such that $\phi(x)|_B = f$. As this is true for every  $B$ and $f:B\to A$ we see that the image of  $\phi$ must be dense in $A^{G}$. As a map on compat spaces maps closed sets on closed sets we see that the image of $\phi$ must be $A^{G}$. 
